...
def nlp_ai(request):
    ...
    # Non-CORS
    if True:
        # Parse request for user_id and other parameters
        ...
        if not user_doc.exists:  # Accessing 'exists' as an attribute, not a method
            ...
            # Initialize user_data to a default dictionary
        ...
        if nlp_calls >= MAX_REQUESTS_PER_WEEK:
            return jsonify({"error": "Usage limit exceeded. You can only make 10 requests per week."}), 403

        # Retrieve recent recommendation
        request_json = request.get_json(silent=True)

        # Get the recommendation_input directly from the request
        recommendation_input = request_json.get('recommendation_input', '')

        # Check if the recommendation_input exists
        if not recommendation_input:
            return jsonify({"error": "No recommendation input found."}), 400  # Use 400 for a bad request
        ...
        for main_word, sub_words in keywords.items():
            # Count occurrences of the main keyword in the prompt
            keyword_counts[main_word] += prompt_lower.count(main_word)
            
            # Count occurrences of each sub-word in the prompt
            for sub_word in sub_words:
                keyword_counts[main_word] += prompt_lower.count(sub_word)

        # Remove any keywords with a count of 0
        ...
        for keyword in keyword_counts:
            if keyword in tips_dict:
                # Ensure that you do not sample more tips than exist in the list
